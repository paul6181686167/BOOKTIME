...
export const recommendationService = {
  ...
  async addRecommendedBook(bookData) {
    try {
      // Si le livre est à paraître (date future), il va dans "upcoming"
      const isUpcoming = (() => {
        const raw = bookData.published_date || bookData.publish_date || '';
        if (!raw) return false;
        const year = parseInt(raw.substring(0, 4), 10);
        if (!year) return false;
        const today = new Date().toISOString().substring(0, 10);
        return raw > today || year > new Date().getFullYear();
      })();
      ...
      const formattedBookData = {
        ...
        status: isUpcoming ? 'upcoming' : 'to_read',
