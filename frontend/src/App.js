...
function MainApp() {
  ...
  // GÉNÉRATEUR DE CARTES SÉRIES AUTOMATIQUE pour la recherche
  const generateSeriesCardsForSearch = useCallback((searchTerm, openLibraryBooks) => {
    if (!searchTerm || searchTerm.length < 3) return [];
    
    const term = searchTerm.toLowerCase().trim();
    const seriesCards = [];
    
    // Base de données des séries populaires avec détection intelligente
    const seriesDatabase = {
      'harry potter': {
        name: 'Harry Potter',
        author: 'J.K. Rowling',
        category: 'roman',
        description: 'La saga magique de Harry Potter, le sorcier le plus célèbre du monde.',
        totalVolumes: 7,
        keywords: ['harry potter', 'potter', 'harry', 'sorcier', 'hogwarts', 'poudlard'],
      ...
      'seigneur des anneaux': {
        ...
        category: 'roman',
        description: 'L\'épopée fantasy légendaire en Terre du Milieu.',
        totalVolumes: 3,
        keywords: ['seigneur des anneaux', 'lord of the rings', 'tolkien', 'anneau', 'terre du milieu'],
      ...
      'one piece': {
        ...
        totalVolumes: 100,
        keywords: ['one piece', 'luffy', 'pirates', 'oda', 'manga'],
        cover_url: 'https://covers.openlibrary.org/b/isbn/9781421536255-L.jpg'
      },
      'naruto': {
        name: 'Naruto',
        ...
        totalVolumes: 72,
        keywords: ['naruto', 'ninja', 'kishimoto', 'sasuke', 'manga'],
        cover_url: 'https://covers.openlibrary.org/b/isbn/9781421900063-L.jpg'
      },
      'dragon ball': {
        name: 'Dragon Ball',
        author: 'Akira Toriyama',
        category: 'manga',
        description: 'Les aventures de Son Goku et la quête des Dragon Balls.',
        totalVolumes: 42,
        keywords: ['dragon ball', 'goku', 'toriyama', 'kamehameha', 'manga'],
        cover_url: 'https://covers.openlibrary.org/b/isbn/9781421526157-L.jpg'
      },
      'astérix': {
        name: 'Astérix',
        author: 'René Goscinny & Albert Uderzo',
        category: 'bd',
        description: 'Les aventures du petit gaulois et de son fidèle ami Obélix.',
        totalVolumes: 39,
        keywords: ['astérix', 'asterix', 'obélix', 'gaulois', 'goscinny', 'uderzo'],
        cover_url: 'https://covers.openlibrary.org/b/isbn/9782012101333-L.jpg'
      },
      'tintin': {
        name: 'Tintin',
        author: 'Hergé',
        category: 'bd',
        description: 'Les aventures du jeune reporter Tintin et de son chien Milou.',
        totalVolumes: 24,
        keywords: ['tintin', 'hergé', 'milou', 'haddock', 'bd'],
        cover_url: 'https://covers.openlibrary.org/b/isbn/9782203001169-L.jpg'
      },
      'lucky luke': {
        name: 'Lucky Luke',
        ...
        totalVolumes: 70,
        keywords: ['lucky luke', 'cowboy', 'morris', 'goscinny', 'western'],
        cover_url: 'https://covers.openlibrary.org/b/isbn/9782884710213-L.jpg'
      },
      'attack on titan': {
        ...
        totalVolumes: 34,
        keywords: ['attack on titan', 'attaque des titans', 'eren', 'titans', 'isayama'],
        cover_url: 'https://covers.openlibrary.org/b/isbn/9781612620244-L.jpg'
      },
      'death note': {
        name: 'Death Note',
        author: 'Tsugumi Ohba & Takeshi Obata',
        category: 'manga',
        description: 'Light Yagami découvre un carnet aux pouvoirs mortels.',
        totalVolumes: 12,
        keywords: ['death note', 'light', 'kira', 'shinigami', 'ohba', 'obata'],
        cover_url: 'https://covers.openlibrary.org/b/isbn/9781421501680-L.jpg'
      }
    };
    
    // Détecter les séries correspondantes
    for (const [seriesKey, seriesData] of Object.entries(seriesDatabase)) {
      // Vérifier si le terme de recherche correspond à cette série
      const isMatch = seriesData.keywords.some(keyword => 
        term.includes(keyword) || keyword.includes(term)
      );
      
      if (isMatch) {
        // Compter les livres de cette série dans les résultats Open Library
        const seriesBooks = openLibraryBooks.filter(book => {
          const bookTitle = (book.title || '').toLowerCase();
          const bookAuthor = (book.author || '').toLowerCase();
          const seriesAuthor = seriesData.author.toLowerCase();
          
          return seriesData.keywords.some(keyword => bookTitle.includes(keyword)) &&
                 (bookAuthor.includes(seriesAuthor.split(' ')[0]) || 
                  bookAuthor.includes(seriesAuthor.split(' ').slice(-1)[0]));
        });
        
        // Créer la carte série
        const seriesCard = {
          id: `series-${seriesKey}`,
          isSeriesCard: true,
          name: seriesData.name,
          title: seriesData.name, // Pour compatibilité
          author: seriesData.author,
          category: seriesData.category,
          description: seriesData.description,
          cover_url: seriesData.cover_url,
          totalVolumes: seriesData.totalVolumes,
          availableBooks: seriesBooks.length,
          books: seriesBooks,
          isFromOpenLibrary: true,
          relevanceScore: 50000, // Score très élevé pour apparaître en premier
          seriesKey: seriesKey,
          // Pour la fiche série
          series_info: {
            total_volumes: seriesData.totalVolumes,
            category: seriesData.category,
            description: seriesData.description,
