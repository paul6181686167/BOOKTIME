...
const AuthorDetailPage = () => {
  ...
  const loadAuthorData = async () => {
    try {
      ...
      // Charger les détails de l'auteur et sa bibliographie en parallèle
      const [authorResponse, booksResponse] = await Promise.all([
        fetch(`${backendUrl}/api/authors/${encodeURIComponent(authorName)}`, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
          }
        }),
        fetch(`${backendUrl}/api/authors/${encodeURIComponent(authorName)}/books`, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
          }
      ...
      if (!authorResponse.ok || !booksResponse.ok) {
  ...
  const handleBookClick = (book) => {
    if (book.user_book_id) {
      ...
      // Pour les livres non dans la bibliothèque, on pourrait ouvrir un modal d'ajout
      toast.info('Ce livre n\'est pas encore dans votre bibliothèque');
    }
  };
  ...
  const addBookToLibrary = async (book) => {
    try {
      ...
      const bookData = {
        ...
        genre: book.subjects || [],
        category: 'roman' // Valeur par défaut
      };
      ...
      // Naviguer vers la fiche du livre
  ...
  const groupedBooks = authorBooks ? groupBooksByDecade(authorBooks.books) : {};
                  ...
                  <div className="mt-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm text-gray-600 dark:text-gray-400">
                        {Math.round((userStats.total_user_books / authorBooks.total_books) * 100)}%
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                      <div 
                        ...
                        style={{ 
                          width: `${Math.min(100, (userStats.total_user_books / authorBooks.total_books) * 100)}%` 
                        }}
                      ></div>
                    </div>
                  </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg">
              <div className="border-b border-gray-200 dark:border-gray-700">
                <nav className="-mb-px flex">
                  ...
                  >
                    📚 Bibliographie ({authorBooks?.total_books || 0})
                  </button>
                  <button
              ...
              <div className="p-6">
                ...
                {activeTab === 'bibliography' && (
                  <div className="space-y-6">
                    {authorBooks && authorBooks.books.length > 0 ? (
