...
// 🆕 SOLUTION C ADAPTÉE POUR LES SÉRIES
export const verifyAndDisplaySeries = async (seriesName, targetCategory, books, loadBooks, loadStats) => {
  const maxAttempts = 3;
  const baseDelayMs = 500;
  const timeoutMs = 5000; // Timeout global 5s
  
  console.log(`🔍 [OPTION C SÉRIES] Vérification série: "${seriesName}" en catégorie "${targetCategory}"`);
  
  const startTime = Date.now();
  
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      console.log(`📚 [OPTION C SÉRIES] Tentative ${attempt}/${maxAttempts} - Chargement données...`);
      
      // Charger données fraîches
      await Promise.all([loadBooks(), loadStats()]);
      
      // Vérifier présence série avec critères stricts
      const seriesFound = books.some(book => 
        book.saga?.toLowerCase().trim() === seriesName.toLowerCase().trim() && 
        book.category === targetCategory &&
      ...
      if (seriesFound) {
        const totalTime = Date.now() - startTime;
        console.log(`✅ [OPTION C SÉRIES] Série trouvée après ${attempt} tentative(s) en ${totalTime}ms`);
        
        // Déclencher retour automatique à la bibliothèque
        setTimeout(() => {
          const backToLibraryEvent = new CustomEvent('backToLibrary', {
            detail: { 
              reason: 'series_added_successfully',
      ...
      if (Date.now() - startTime > timeoutMs) {
        console.log(`⏰ [OPTION C SÉRIES] Timeout global atteint (${timeoutMs}ms)`);
        break;
      }
      ...
      if (attempt < maxAttempts) {
        ...
        console.log(`⏳ [OPTION C SÉRIES] Attente ${delay}ms avant tentative ${attempt + 1}...`);
        await new Promise(resolve => setTimeout(resolve, delay));
      }
      
    } catch (error) {
      console.error(`❌ [OPTION C SÉRIES] Erreur tentative ${attempt}:`, error);
      if (attempt === maxAttempts) throw error;
    }
  }
  
  // Si aucune tentative n'a réussi, afficher message d'aide
  console.log(`❌ [OPTION C SÉRIES] Série non trouvée après ${maxAttempts} tentatives`);
  toast.error('Série ajoutée mais pas encore visible. Actualisez manuellement si nécessaire.', {
    duration: 5000
  });
...
// 🆕 FONCTION AJOUT SÉRIE DEPUIS OPEN LIBRARY
export const handleAddSeriesFromOpenLibrary = async (seriesData, {
  books,
  addingBooks,
  setAddingBooks,
  activeTab,
  ...
  // Empêcher les clics multiples sur la même série
  const seriesKey = `series_${seriesData.series_name}_${seriesData.author}`;
  if (addingBooks.has(seriesKey)) {
    return; // Si la série est déjà en cours d'ajout, ne rien faire
  }

  try {
    // Marquer la série comme en cours d'ajout
    setAddingBooks(prev => new Set([...prev, seriesKey]));
    
    const token = localStorage.getItem('token');
    const backendUrl = process.env.REACT_APP_BACKEND_URL || 'http://localhost:8001';
    
    // PLACEMENT INTELLIGENT : Déterminer la catégorie automatiquement
    const categoryBadge = getCategoryBadgeFromSeries ? getCategoryBadgeFromSeries(seriesData) : null;
    let targetCategory = categoryBadge?.key || seriesData.category || activeTab;
    ...
    const response = await fetch(`${backendUrl}/api/openlibrary/import`, {
      ...
      body: JSON.stringify({
        series_data: {
          ...
          total_volumes: seriesData.total_volumes || 0,
    ...
    if (response.ok) {
      ...
      toast.success(`Série "${seriesData.series_name}" ajoutée avec succès ! 📚`, {
        duration: 2000
      });
      
      // ✅ SOLUTION ROBUSTE OPTION C : Vérification intelligente et retour bibliothèque
      await verifyAndDisplaySeries(
      ...
      if (response.status === 409) {
        ...
        toast.error(errorData.detail || 'Erreur lors de l\'ajout de la série');
      }
    }
  } catch (error) {
    ...
    // Retirer la série de la liste des ajouts en cours
    setAddingBooks(prev => {
      const newSet = new Set(prev);
