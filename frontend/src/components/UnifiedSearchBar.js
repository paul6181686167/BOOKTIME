...
const UnifiedSearchBar = React.memo(({ 
  ...
  // Synchroniser l'état local avec la prop searchTerm UNIQUEMENT si différent
  useEffect(() => {
    if (searchTerm !== localSearchTerm) {
      setLocalSearchTerm(searchTerm || '');
    }
  }, [searchTerm]); // Retirer localSearchTerm des dépendances pour éviter les boucles
  ...
  // Pas de suggestions Open Library pendant la saisie : chaque frappe lançait
  // une recherche OL (5–8 s) qui saturait le backend et cassait le login.
  // Les suggestions locales (bibliothèque) restent ; OL ne part qu'à Entrée.
  useEffect(() => {
    setUniversalResults([]);
  }, [localSearchTerm]);
