#!/usr/bin/env python3
"""
Modified Authentication test script for the BOOKTIME API
This script tests the new authentication system that uses only first_name and last_name
"""
...
BACKEND_URL = "https://0cc916bc-2daf-4e2e-a8b7-15a28a66042a.preview.emergentagent.com"
...
class BooktimeModifiedAuthTest(unittest.TestCase):
    """Test suite for the modified Booktime API Authentication"""
    ...
    def test_1_user_registration(self):
        """Test user registration with first_name and last_name only"""
        ...
        print("✅ User registration with first_name and last_name works correctly")

    def test_2_register_existing_user(self):
    ...
    def test_3_register_missing_fields(self):
        ...
        invalid_user = {"last_name": self.test_user_data["last_name"]}
        response = requests.post(f"{API_URL}/auth/register", json=invalid_user)
        self.assertEqual(response.status_code, 400)
        
        # Test missing last_name
        invalid_user = {"first_name": self.test_user_data["first_name"]}
        response = requests.post(f"{API_URL}/auth/register", json=invalid_user)
        self.assertEqual(response.status_code, 400)
    ...
    def test_4_user_login(self):
        ...
        # Login with first_name and last_name
        login_data = {
        ...
        print("✅ User login with first_name and last_name works correctly")

    def test_5_login_incorrect_name(self):
        """Test login with incorrect name"""
        # First register a user
        response = requests.post(f"{API_URL}/auth/register", json=self.test_user_data)
        self.assertEqual(response.status_code, 200)
        
        # Login with incorrect first_name
        login_data = {
            "first_name": "Wrong" + self.test_user_data["first_name"],
            "last_name": self.test_user_data["last_name"]
        }
        ...
        # Login with incorrect last_name
        login_data = {
            "first_name": self.test_user_data["first_name"],
            "last_name": "Wrong" + self.test_user_data["last_name"]
        }
        
        response = requests.post(f"{API_URL}/auth/login", json=login_data)
        self.assertEqual(response.status_code, 401)
        
        print("✅ Login with incorrect name fails as expected")

    def test_6_login_nonexistent_user(self):
    ...
    def test_7_get_current_user(self):
        ...
        # Verify that email is not returned
        self.assertNotIn("email", user)
        
        print("✅ Get current user info works correctly")

    def test_8_get_user_invalid_token(self):
    ...
    def test_9_get_user_no_token(self):
    ...
    def test_10_protected_books_endpoint(self):
    ...
    def test_11_protected_stats_endpoint(self):
    ...
    def test_12_user_specific_books(self):
...
def run_tests():
    ...
    suite = unittest.TestLoader().loadTestsFromTestCase(BooktimeModifiedAuthTest)
