#!/usr/bin/env python3
"""
Ultra Harvest 100k AutoExpansion OpenLibrary - Focus Auteurs Découverts (Suite)
Mission: Analyser les 34 auteurs restants de la Session 81.31
Date: 12 Mars 2025
"""
...
# Auteurs découverts dans Session 81.30 + extensions (34 auteurs restants)
REMAINING_AUTHORS = [
    # Auteurs 31-64 (pas analysés dans Session 81.31)
    "Kiersten White", "Rainbow Rowell", "Stephanie Perkins", "Anna Todd", 
    "Colleen Hoover", "Cassandra Jean", "Brandon Sanderson", "Patrick Rothfuss",
    "Jim Butcher", "Kim Harrison", "Patricia Briggs", "Kelley Armstrong",
    "Laurell K. Hamilton", "Anne Rice", "Charlaine Harris", "Sherrilyn Kenyon",
    "Gena Showalter", "Kresley Cole", "Stephanie Hans", "Rebecca Ross",
    "Adrienne Tooley", "Akshaya Raman", "Chloe Gong", "Xiran Jay Zhao",
    "Erin A. Craig", "Hafsah Faizal", "Namina Forna", "Tracy Wolff",
    "Jennifer Lynn Barnes", "Kerri Maniscalco", "Sarah Maas", "J. L. Armentrout",
    "Jennifer Armentrout", "J.L. Armentrout"
]

# Séries connues par auteur (pour éviter doublons)
KNOWN_AUTHOR_SERIES = {
    "Brandon Sanderson": ["Mistborn", "Stormlight Archive", "Warbreaker", "Elantris"],
    "Patrick Rothfuss": ["The Name of the Wind", "Kingkiller Chronicle"],
    "Jim Butcher": ["Dresden Files", "Codex Alera", "Cinder Spires"],
    "Kim Harrison": ["The Hollows", "Madison Avery"],
    "Patricia Briggs": ["Mercy Thompson", "Alpha and Omega"],
    "Kelley Armstrong": ["Women of the Otherworld", "Darkest Powers"],
    "Laurell K. Hamilton": ["Anita Blake", "Meredith Gentry"],
    "Anne Rice": ["Vampire Chronicles", "Mayfair Witches", "Lives of the Mayfair Witches"],
    "Charlaine Harris": ["Sookie Stackhouse", "Harper Connelly", "Aurora Teagarden"],
    "Sherrilyn Kenyon": ["Dark-Hunter", "Chronicles of Nick", "Dream-Hunter"],
    "Gena Showalter": ["Lords of the Underworld", "Angels of the Dark"],
    "Kresley Cole": ["Immortals After Dark", "The Arcana Chronicles"],
    "Kiersten White": ["Paranormalcy", "The Dark Divine", "Slayer"],
    "Rainbow Rowell": ["Simon Snow", "Eleanor & Park"],
    "Colleen Hoover": ["It Ends with Us", "Ugly Love", "November 9"],
    "Tracy Wolff": ["Crave", "Crush", "Covet"],
    "Jennifer Lynn Barnes": ["The Naturals", "Little White Lies"],
    "Kerri Maniscalco": ["Stalking Jack the Ripper", "Kingdom of the Wicked"]
}
...
class UltraHarvestRemainingAuthors:
    def __init__(self):
        self.setup_logging()
    ...
    def setup_logging(self):
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler('/app/backend/ultra_harvest_remaining_authors.log'),
    ...
    def setup_author_detection_patterns(self):
        ...
        self.series_patterns = [
            ...
            # Patterns urban fantasy/paranormal
            r'(.+?)\s+(?:hunter|slayer|guardian|protector)\s*(\d+)',
            r'(.+?)\s+(?:dark|shadow|night|blood|moon)\s*(\d+)',
    ...
    def search_author_books(self, author: str) -> List[Dict]:
        ...
        if " " in author:
            ...
            if len(parts) >= 2:
                ...
                # Sans initiales
                if len(parts) == 3:  # Prénom Milieu Nom
                    search_variants.append(f'author:"{parts[0]} {parts[2]}"')
        ...
        for book_author in book_authors:
            ...
            if len(target_parts) >= 2 and len(book_parts) >= 2:
                ...
                if target_parts[-1] == book_parts[-1]:
                    # Même première lettre du prénom ou prénom complet
                    if (target_parts[0][0] == book_parts[0][0] or 
                        target_parts[0] == book_parts[0]):
                        return True
    ...
    def calculate_author_confidence(self, book: Dict, series_name: str, volume_num: str, author: str) -> int:
        ...
        if author in REMAINING_AUTHORS:
            confidence += 20
        ...
        if subjects:
            ...
            relevant_subjects = [
                ...
                'romance', 'mystery', 'thriller', 'adventure', 'paranormal',
                'urban fantasy', 'contemporary fiction'
            ]
    ...
    def calculate_author_score(self, book: Dict, author: str) -> int:
        ...
        if subjects:
            ...
            relevant = [
                'fiction', 'fantasy', 'science fiction', 'young adult', 
                'romance', 'paranormal', 'urban fantasy', 'contemporary'
            ]
        ...
        if publishers:
            major_publishers = [
                ...
                'simon schuster', 'hachette', 'scholastic', 'tor', 'daw'
            ]
    ...
    def get_author_match_reasons(self, book: Dict, series_name: str, volume_num: str, author: str) -> List[str]:
        ...
        try:
            vol_int = int(volume_num)
            reasons.append(f"remaining_author_volume_{vol_int}")
        except:
            reasons.append("remaining_author_volume_pattern")
        
        # Auteur découvert
        if author in REMAINING_AUTHORS:
            reasons.append(f"remaining_author_{author.lower().replace(' ', '_')}")
        ...
        if subjects:
            ...
            elif 'paranormal' in subject_text:
                reasons.append("paranormal_genre")
    ...
    def run_remaining_authors_harvest(self):
        """Exécuter Ultra Harvest sur auteurs restants"""
        self.logger.info("🚀 Démarrage Ultra Harvest Auteurs Restants (34 auteurs)")
        self.logger.info(f"Auteurs ciblés: {len(REMAINING_AUTHORS)} auteurs restants")
        
        for i, author in enumerate(REMAINING_AUTHORS, 1):
            if self.total_api_calls >= MAX_REQUESTS:
                self.logger.info("Limite API atteinte, arrêt")
                break
                
            self.logger.info(f"[{i}/{len(REMAINING_AUTHORS)}] Analyse auteur: {author}")
        ...
        # Rapport final
        self.generate_remaining_authors_final_report()

    def generate_remaining_authors_final_report(self):
        """Générer rapport final auteurs restants"""
        duration = time.time() - self.session_start_time
        
        self.logger.info("\n" + "="*80)
        self.logger.info("🎯 RAPPORT FINAL - ULTRA HARVEST AUTEURS RESTANTS")
        self.logger.info("="*80)
        self.logger.info(f"Auteurs analysés: {self.authors_processed}/{len(REMAINING_AUTHORS)}")
        ...
        if self.new_series:
            self.logger.info("\n🏆 TOP 15 DÉCOUVERTES AUTEURS RESTANTS:")
        ...
        if self.new_series:
            ...
            self.logger.info("\n📊 RÉPARTITION PAR AUTEUR RESTANT (Top 10):")
            top_authors = sorted(author_counts.items(), key=lambda x: x[1], reverse=True)[:10]
            for author, count in top_authors:
                self.logger.info(f"  {author}: {count} séries")
        
        # Sauvegarde
        self.save_remaining_authors_results()

    def save_remaining_authors_results(self):
        """Sauvegarder résultats auteurs restants"""
        if not self.new_series:
            self.logger.info("Aucune nouvelle série auteur restant à sauvegarder")
            return
        ...
        for series in self.new_series:
            new_entry = {
                ...
                "languages": ["en", "fr"],
                "description": f"Série de {series.author} découverte via Ultra Harvest Auteurs Restants (confiance {series.confidence}%, score auteur {series.author_score})",
                ...
                "discovery_method": "ultra_harvest_remaining_authors_50",
                "discovery_date": time.strftime("%Y-%m-%d")
            }
            existing_series.append(new_entry)
        
        # Sauvegarder
        backup_file = f'/app/backend/data/backup_before_remaining_authors_{int(time.time())}.json'
        ...
        self.logger.info(f"✅ Sauvegarde auteurs restants réussie: {len(self.new_series)} nouvelles séries ajoutées")
        self.logger.info(f"Backup créé: {backup_file}")

if __name__ == "__main__":
    harvester = UltraHarvestRemainingAuthors()
    harvester.run_remaining_authors_harvest()