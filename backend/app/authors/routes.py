...
async def get_openlibrary_author_works(author_name: str):
    ...
    try:
        ...
        works_params = {"limit": 100, "fields": "title,first_publish_date,key,description,series,subjects"}
...
def filter_openlibrary_works(works_entries, author_name):
    """Filtrer les œuvres OpenLibrary en conservant l'info de série"""
    filtered_works = []
    seen_titles = set()
    import re
    ...
    try:
        for work in works_entries:
            ...
            # ✅ Récupérer le champ série natif d'OpenLibrary
            # OpenLibrary retourne "series" comme liste de noms de séries
            series_list = work.get("series", [])
            series_name = series_list[0] if series_list else None
            
            # Si pas de champ série natif, essayer de détecter via les subjects
            if not series_name:
                subjects = work.get("subjects", [])
                for subject in subjects:
                    if isinstance(subject, str) and any(k in subject.lower() for k in ["series", "saga"]):
                        series_name = subject
                        break
            
            filtered_works.append({
                "title": title,
                "type": "individual",  # sera reclassifié dans organize_author_works
                "source": "openlibrary",
                "author": author_name,
                "year": year,
                ...
                "series_name": series_name  # ← champ clé pour le regroupement
            })
            
            if len(filtered_works) >= 50:
                break
...
def organize_author_works(works):
    ...
    for work in works:
        sources[work.get("source", "openlibrary")] = sources.get(work.get("source", "openlibrary"), 0) + 1
        
        # ✅ Utiliser series_name natif OpenLibrary en priorité
        series_name = work.get("series_name")
        
        # Fallback : si le type était "series" (détection Wikipedia)
        if not series_name and work.get("type") == "series":
            series_name = work["title"]
        
        if series_name:
            # Ce livre appartient à une série → le placer dessous
            if series_name not in series_works:
                series_works[series_name] = {
                    "name": series_name,
                    "type": "series",
                    "author": work["author"],
                    "source": work.get("source", "openlibrary"),
                    "description": "",
                    "books": []
                }
            series_works[series_name]["books"].append({
                "title": work["title"],
                "year": work.get("year"),
                "key": work.get("key", ""),
                "description": work.get("description", "")
    ...
    # Trier les livres de chaque série par année
    for series in series_works.values():
        series["books"].sort(key=lambda x: x.get("year") or 9999)
    
    individual_works.sort(key=lambda x: x.get("year") or 9999)
