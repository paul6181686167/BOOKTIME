...
class ExportImportAPITest(unittest.TestCase):
    ...
    def test_export_json(self):
        ...
        response = requests.post(f"{API_URL}/books", json=book_data, headers=self.auth_headers)
        self.assertEqual(response.status_code, 200)
        ...
        # Check that the books data contains our test book
        books = data["books"]
        self.assertGreaterEqual(len(books), 1)
        
        # Verify book title
        book_titles = [book["title"] for book in books]
        self.assertIn("Test Export Book", book_titles)
    ...
    def test_export_csv(self):
        ...
        self.assertIn("Test CSV Export", csv_data)
        self.assertIn("Test Author", csv_data)
        self.assertIn("roman", csv_data)
...
Test Import Book 3,Test Author,manga,completed,4,Great book!,300,300"""
        ...
        # Verify at least some books were imported
